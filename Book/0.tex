...
%\usepackage{array}
%\begin{document}\input{defs0}\input{miradefs}\setcounter{chapter}{-1}\setcounter{page}{1}
...
\medskip\noindent
The book is also available as a \href{https://github.com/simonjohnthompson/haskellcraft/blob/main/Book/root.pdf}{PDF}, as part of the \href{https://github.com/simonjohnthompson/haskellcraft/}{repository holding the source code for the text}.
} % end beware
...
\begin{itemize}
\item 
Functional languages are used to build secure, error-free, systems that are used in practice by thousands of people every day. For example, the \href{https://xmonad.org}{\emph{Xmonad}} window manager for linux systems is written entirely in Haskell, and the \href{https://github.com/GaloisInc/cryptol}{\emph{Cryptol}} language, used to design cryptographic systems in C and VHDL, is itself implemented using Haskell. At a larger scale, the investment bank Standard Chartered runs its Markets trading division on a Haskell dialect spanning millions of lines of code, the fintech company Mercury processes hundreds of billions of dollars a year in transactions on a Haskell backend, and \href{https://docs.cardano.org/developer-resources/smart-contracts/plutus}{Plutus}, the smart-contract language for the Cardano blockchain, is a dialect of Haskell, and indeed the code for the Cardano blockchain node is itself written in Haskell.
\item
...
\item
In fact, most modern languages, including Java, C and C++, are \emph{already} functional, in the sense that they contain `first class' functions, or functions as data, defined using a lambda. Users of these languages stress the value of having access to higher-order functions when defining as general an API as possible for a library. When concurrent threads are to be used, an immutable data type can provide a safe and effective approach: as observed by Pat Helland, \href{https://queue.acm.org/doi/10.1145/2857274.2884038}{\emph{immutability changes everything}}.
\item
...
changes and modifications. This text is written in Haskell 2010, which remains the latest official language standard: in practice it has proved a remarkably stable base, with GHC instead evolving through language extensions -- curated since 2021 into \emph{language editions} such as \texttt{GHC2021} and \texttt{GHC2024} -- so the parts of the language discussed here remain stable in current GHC.
...
available from the \href{https://www.haskell.org}{Haskell home page}.

The \emph{Haskell Foundation}, established in 2020, now provides much of
the ecosystem's shared infrastructure: it funds maintenance of core tools
and libraries, and runs an annual community survey tracking how Haskell
is actually used in practice
(\href{https://discourse.haskell.org/t/state-of-haskell-2025-results/13755}{State of Haskell 2025}).
A Haskell Foundation working group has recently begun work on a revised
Haskell 2010 Report, the first update to the standard in over a decade.
...
programming. 
\item
Other support materials appear at the book's \href{https://github.com/simonjohnthompson/haskellcraft}{GitHub repository}
\end{itemize}
...
types, first introduced in Chapter \ref{io}. Chapter \ref{io-programming} revises this, and illustrates some larger-scale examples, including an
...
action-based programming of a number of different flavours, examined in
Chapter \ref{actions}.
...
background materials, can be downloaded from the book's \href{https://github.com/simonjohnthompson/haskellcraft}{GitHub repository}.
...
with mdBook, with the source for both available from the book's
\href{https://github.com/simonjohnthompson/haskellcraft}{GitHub repository}.
\item
...
Chapter \ref{getStart} and its GHCi transcripts and screenshots have
been brought up to date to match. Readers who want to try the book's
code without installing anything can use the accompanying dev
container (GitHub Codespaces or Gitpod).
\item
...
\item
Guidance on finding libraries and getting help, in Section
\ref{HaskellLibs}, has been refreshed: Hackage, Stackage snapshots,
Haddock and Hoogle for libraries; the Haskell Discourse, Haskell Cafe
and Stack Overflow for questions.
\item
Dead links to \texttt{haskellcraft.com} have been retired, with that
material folded into the book and its GitHub repository.
\item
A number of case-study programs have been refreshed: the graphical
\emph{Minesweeper} example in Chapter \ref{further} now uses the
actively-maintained \texttt{gloss} library in place of a graphics
package that no longer exists, and several figures -- including the
class-hierarchy diagram in Chapter \ref{classes} -- have been
replaced with freely-licensed originals.
\end{itemize}
...
\item
A collection of \emph{project suggestions} has been added as an appendix.
\end{itemize}
...
\end{description}
The case studies in the book are designed to illustrate particular points and constructs  as well as to give examples of larger programs than single function definitions.\label{CaseStudies}
\begin{description}
\item[Pictures]
...
Moving forward to 2026, I would reiterate all my thanks, and acknowledge colleagues and students at E\"{o}tv\"{o}s Lor\'{a}nd University in Budapest who have given me abundant feedback on courses that I have taught there; in particular Viktor Bense went above and beyond in providing invaluable feedback on this text and my online lectures.
...
\noindent
\emph{Simon Thompson}\\
\emph{Canterbury, December 2010 and September 2026}\\
}
