...
%\usepackage{array}
%\begin{document}\input{defs0}\input{miradefs}\setcounter{chapter}{18}\setcounter{page}{1001}
...
We can get detailed performance information by \emph{compiling} programs using \texttt{cabal} and passing the right parameters to the resulting executable. For the simple examples here, we define the expressions we are interested in Figure \ref{sumDefs}; the three variants -- \texttt{PerformanceI.hs}, \texttt{PerformanceIA.hs} and \texttt{PerformanceIS.hs}, in \texttt{Code/Craft3e/Chapter20} -- are already declared as three separate executables in \texttt{Craft3e.cabal}. Building all three, from the \texttt{Code/Craft3e} directory, is a single command:
\begin{alltt}
cabal build performanceI performanceIA performanceIS
\end{alltt}
which produces three executables, \texttt{performanceI}, \texttt{performanceIA} and \texttt{performanceIS}, one for each variant of the \texttt{main} program -- no renaming needed, since \texttt{cabal} already gives each its own name.
...
The performance of \texttt{performanceIA} is twice as bad (!), but for \texttt{performanceIS}, as shown in  Figure \ref{perfISreport}, we see something much better. How do the two reports compare?
\begin{titemize}
\item
