...
\href{https://www.haskell.org/}{Haskell home page}.
...
\item
\emph{Real World Haskell} \cite{realWorldHaskell}, available in print and, via the Internet Archive's lending library, online too. It starts from the beginning, but goes at a faster pace than we did here, and so covers a number of practical aspects of Haskell which we weren't able to do, such as the foreign function interface. Its code examples predate Stack and Stackage, so some will need updating to build with modern tooling, but it remains a good follow-on read, particularly for its FFI coverage.
\item
\emph{Learn You a Haskell for Great Good!} \cite{learnYou} introduces Haskell for those who are familiar with imperative programming, and is written in a very approachable style; it complements what we have covered here, as well as going into some topics -- like monads and zippers -- in much more detail. It began as a website and was later published as a printed book too; a community-maintained continuation of \href{https://learnyouahaskell.github.io/}{the website} lives on.
\item
\emph{Haskell Programming from First Principles} \cite{hpfp} is a substantial, thorough self-study text, widely recommended within the Haskell community as a comprehensive next step after an introductory book like this one.
\item
\emph{Effective Haskell} \cite{effectiveHaskell} is a more recent, practically-focused follow-on, covering the modern tooling ecosystem -- Cabal, Stack, testing and packaging -- alongside the language itself.
\item
...
of the libraries in detail. As we explained in Chapter \ref{progLists}, they are documented in Haddock, and this documentation is available online for type- and name-based search in Hoogle.
...
language like Haskell will not stand still, even though the formal Haskell 2010 standard itself has: it has proved a far more stable base than anyone expected. Instead, the language has evolved through GHC's own extensions, curated since 2021 into \emph{language editions} such as \texttt{GHC2021} and \texttt{GHC2024}, incorporating features which have become \emph{de facto} parts of the language through their implementation in GHC, as well as more advanced features, particularly in the type system. A Haskell Foundation working group has now begun work on a revised Haskell 2010 Report, the first update to the language standard in well over a decade.
...
found on the web. This text itself \href{https://github.com/simonjohnthompson/haskellcraft}{has a home page}.
The Haskell home page, Figure \ref{HaskellDotOrg}, is \href{https://www.haskell.org/}{online},
...
The Haskell community includes online forums (Discourse, IRC, mailing lists), news (Reddit), blogging (Planet Haskell) and help (Stack Overflow). The \href{https://haskellweekly.news/}{\emph{Haskell Weekly}} newsletter and podcast gives a regular summary of Haskell-related software releases, blog entries and other news, and is syndicated to Planet Haskell.

The Haskell language was named in honour of Haskell Brooks Curry. A
\href{https://mathshistory.st-andrews.ac.uk/Biographies/Curry/}{short biography and photograph of Curry}\index{Curry,
Haskell B.!biography} can be found online.
...
\citeN{workingml2e} provides an introduction. The latest member of the ML family of languages, is F\# \cite{Fsharp}, which is now open source and cross-platform as part of .NET. Since is possible to model
...
Two more recent languages carry Haskell's influence directly onto the web front end. \emph{PureScript} \cite{PureScript} is a strict Haskell-like language that compiles to JavaScript, sharing much of Haskell's syntax and type system, and is a natural next step for readers who enjoyed this book's browser-based implementation of pictures. \emph{Elm} \cite{Elm} takes a related but more restricted approach, trading some of Haskell's expressiveness for a simpler, beginner-friendly language with an emphasis on especially friendly compiler error messages and a distinctive application architecture (`The Elm Architecture') that has gone on to influence state management in other web frameworks.
...
A more recent, and rather different, LISP dialect is \emph{Clojure} \cite{Clojure}, which runs on the Java Virtual Machine (and, via ClojureScript, in the browser), and pairs LISP's traditional dynamic typing and macro system with a strong, pragmatic emphasis on immutable data structures for building concurrent, JVM-hosted systems.

Erlang is a concurrent, fault-tolerant, distributed language, based on a functional programming core. Erlang \cite{progErlang,erlangProg} was developed within Ericsson, and as well as its use in telecoms applications, has applications in the financial sector, and to high-availability distributed systems in general. A more recent language built on the same Erlang virtual machine (the BEAM) is \emph{Elixir} \cite{Elixir}, which pairs Erlang's concurrency and fault-tolerance model with a more modern, Ruby-influenced syntax; its \emph{Phoenix} web framework has become particularly well regarded for building highly concurrent, real-time web applications.

Some of Haskell's ideas have also reached mainstream systems programming. \emph{Rust} \cite{Rust} is not itself a functional language, but its algebraic data types (its \texttt{enum}s), pattern matching, and its \texttt{Option} and \texttt{Result} types for handling absence and failure without exceptions are all directly descended from the ML/Haskell tradition, and its ownership-based type system enforces, at compile time, much of the discipline around mutation and aliasing that functional languages achieve through immutability.
...
Research in functional programming is reported in the \href{https://jfp.episciences.org/}{\emph{Journal
of Functional Programming}}, and at the annual
\href{https://www.icfpconference.org/}{International Conference in Functional Programming}.
Each year there is a \href{https://www.haskell.org/haskell-symposium/}{one-day symposium devoted to research in Haskell},
...
Looking back at this from 2026, I'd call the prediction half right. Chips with \emph{thousands} of cores have not become mainstream, general-purpose hardware; instead the industry's appetite for parallelism has largely been satisfied by GPUs and other accelerators, with multicore CPUs settling at a far more modest scale than I imagined. Even so, Haskell's clean models for concurrency have matured well: GHC's lightweight threads, the \texttt{async} library and software transactional memory (STM) are now a standard part of writing concurrent Haskell, and \texttt{LinearTypes}, added to GHC in 2021, gives the type system a direct line to the resource-management concerns behind the second edition's type-system prediction above. That prediction's other thread, type-level programming, has also kept moving: kind polymorphism, promoted data types and richer type families let programmers express more and more at the type level, and libraries like \texttt{singletons} bridge the gap between types and terms. Full \emph{Dependent Haskell} -- with genuine dependent functions and pairs, rather than these encodings -- remains a live research and design goal rather than a shipped reality, but GHC is closer to it than it was in 2011.

The intervening years have also brought developments I could not have anticipated. Haskell is now used at genuine industrial scale: as the Preface describes, \emph{Plutus}, the smart-contract language of the Cardano blockchain, is a Haskell dialect, and companies including Standard Chartered and Mercury run substantial production systems written in Haskell. \emph{Effect systems} such as \texttt{effectful} and \texttt{polysemy} have emerged as a popular alternative to monad transformer stacks for structuring effectful code. And the Haskell Foundation, founded in 2020 and also discussed in the Preface, now gives the language the kind of dedicated organisational backing for tooling and libraries it previously lacked.
