...
\centering\includegraphics{Pictures/fig5-1.pdf}
...
\textbf{Hackage} is an online repository for Haskell programs, bundled as a \textbf{package}. The simplest package is a collection of Haskell modules, but more complex packages can contain C code, documentation, test cases and so forth. Moreover,  
Hackage has the great advantage that it keeps track of dependencies between packages and indeed package versions. 
...
\medskip
\textbf{Packages} have a globally unique \textbf{package name} and a \textbf{version number}, such as \texttt{HaRe-0.6.0.1}. Each package will \textbf{expose} some of the Haskell modules it contains. Crucially, the package will state which other packages it depends on, typically in as general as possible a way. The HaRe package mentioned earlier has this dependency: \texttt{base >4 \&\& <= 5} so that it can use \emph{any} version of the \texttt{base} package in that range, including `fractional' versions, of course. Further details of how packages are specified is available at the Hackage website.
\begin{alltt}
http://hackage.haskell.org/
\end{alltt}
A typical package description page in Hackage  is shown in Figure \ref{HackagePic}.
}
\end{figure}

\begin{figure}[t]
\beware{Cabal}{\label{cabal}\index{Cabal}
\textbf{Cabal} is a command-line tool for installing packages -- and the packages that they depend on -- in your system. Cabal is itself installed as a part of the Haskell Platform. As a user you'll need to know these cabal commands.
...
\\Install the package \texttt{pkg}; you can also supply a list of packages here.
\item{\mbox{\texttt{cabal install foo-1.0	}}} 
\\Install specific version of a package.
\item{\mbox{\texttt{cabal install 'foo < 2'}}}	
...
Libraries are found in the Haskell 2010 standard, including the \texttt{Prelude}, in the Haskell Platform and on the Hackage database. 
...
\item[The Haskell Platform]\index{Haskell Platform}
The Haskell Platform is a bundled distribution of GHCi, some standard tools, including Cabal for building, distributing and downloading libraries and the \texttt{Hsc2hs} preprocessor to assist in Haskell/C interworking.
...
HackageDB provides an online repository for sharing and distributing Haskell packages and libraries. There are 2787 packages on Hackage at the time of writing\footnote{Early August 2010}, a testament to the strength and engagement of the Haskell developer community. The variety and number of packages make it impossible to give a summary of what's there: you'll need to consult the documentation to find out more. We turn to looking at that now. 
\end{description}
...
You can find out about the Haskell libraries in a number of different places, some bundled with GHC and the Haskell Platform locally on your machine, and others online. Much of the online documentation is generated using the \textbf{Haddock}\index{Haddock} system, which itself comes bundled with the Haskell Platform. Haddock documents are hyperlinked for ease of navigation, and also provide links from the documentation into the source code, so that you don't have to look at that separately.
...
\end{center} 
\caption{The HackageDB description of the \texttt{agum} package}
\label{HackagePic}
\end{figure}
...
\end{center} 
\caption{Haddock documentation for the \texttt{agum} package}
\label{Haddock}
\end{figure}
...
\end{alltt}
On linux systems the default installation is in \texttt{/usr} or \texttt{/usr/local}.
On Windows systems there are links to the documentation in the \texttt{Haskell Platform} programs group accessible from the \texttt{All Programs} menu.  
\item[Libraries]\index{documentation!libraries}
Haddock documentation for many libraries is available from the \texttt{Libraries} link on the system documentation page, or directly at
\begin{alltt}
file:///usr/share/doc/ghc/html/libraries/index.html
\end{alltt}
This is also inked from the \texttt{Haskell Platform} programs group on Windows.
\item[Prelude]\index{documentation!prelude}
In particular, documentation for the \texttt{Prelude}, including descriptions of and examples for many of the functions, is available within the library documentation, or directly at
\begin{alltt}
file:///usr/share/doc/ghc/html/libraries/base-XXXXX/Prelude.html
\end{alltt}
Here \texttt{XXXXX} is the particular version of the base libraries: for the Haskell Platform at the time of writing it is \texttt{base-4.2.0.2}.
...
\end{alltt}
which is listed by category, but also searchable. A typical package page is shown in Figure \ref{HackagePic}: documentation for the modules making up the package is linked from the list of modules below the \textbf{Module} header, towards the bottom of the page; an example is shown in Figure \ref{Haddock}.
\item[Hoogle search]\label{hoogle}\index{documentation!Hoogle}\index{Hoogle}
Hoogle allows you to search many of the standard libraries: what is cool about Hoogle is that you can \textbf{search by type} as well as by name. Narrowing down a search by type can lead you very quickly to your answer, or at least eliminate a lot of ``noise'' in a search.  
\begin{alltt}
http://www.haskell.org/hoogle/
\end{alltt}
See Figure \ref{Hoogle} for example results. Be aware, though, that Hoogle doesn't cover all the packages in Hackage.
\item[Hayoo! search]\index{documentation!Hayoo\symbol{33}}\index{Hayoo\symbol{33}}
Hayoo!\ \emph{does} search the whole of Hackage, but it is a string-based search only.
\begin{alltt}
http://holumbus.fh-wedel.de/hayoo/hayoo.html
\end{alltt}
\marginpar{
\includegraphics[width=3cm]{Pictures/Hayoo.png}
}
...
You can go online to ask questions: there's a really lively community on the Haskell Cafe mailing list, \texttt{haskell-cafe@haskell.org}: you can find out more about how to join this list in the {Communities} section on the homepage of the Haskell website \texttt{www.haskell.org}: you can also find links to the Haskell \emph{IRC channel} and Haskell on \emph{Stack Overflow} here too. 
\item[Online texts]
Finally, you can find a number of texts online, including \emph{Learn You a Haskell for Great Good}, \texttt{http://learnyouahaskell.com/} and the \emph{Haskell wikibook}, \texttt{http://en.wikibooks.org/wiki/Haskell}.
\end{description}
...
\item
Correct the property \texttt{prop_AboveFlipH} given earlier.
