...
A graphical interface to the same game is also kept in the repository, in \codelink{https://github.com/simonjohnthompson/haskellcraft/blob/main/Code/Craft3e/Minesweeper/Graphical/GraphicMine.hs}{\texttt{Minesweeper/Graphical/GraphicMine.hs}}. An earlier version of this file, built using a Haskell graphics library that no longer exists, has been rewritten to use the actively-maintained \texttt{gloss} package instead: click a cell to reveal it, right-click to flag it, press \texttt{n} for a new game or \texttt{a} for a deduction assist, and press \texttt{Esc} to quit. Run it with \texttt{cabal run minesweeperGraphical}, the one part of the \texttt{Craft3e} package that isn't pulled in by an ordinary \texttt{cabal install Craft3e}, since it depends on \texttt{gloss} and the OpenGL/GLUT libraries it in turn needs.
