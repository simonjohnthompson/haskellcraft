...
%\begin{document}\input{defs0}\input{miradefs}\appendix
...
At the University of Kent we have tried to incorporate more explicit advice about problem solving strategies into our programming teaching, based on Polya's classic \emph{How to Solve It}. Two handouts developed for this are available online: \emph{How To Program It}, \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/HowToProgIt.html} (PDF: \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/howToProgIt.pdf}), which discusses general strategies for writing small-scale programs; and \emph{Programming It In Haskell}, \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/ProgInHaskell.html} (PDF: \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/progInHaskell.pdf}), which shows how these ideas are used in writing small- to medium-size Haskell programs. The approach is also described in the paper \emph{A problem solving approach in teaching functional programming} \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/ProbSolvInHas.ps.gz}, published in the proceedings of the First International Conference on Declarative Programming Languages in Education.
...
This example shows how the problem solving approach applies in Haskell, and how it can help you get started on a problem which at first looks more complicated than it turns out to be. An executable version of the program is available at \url{http://www.cs.kent.ac.uk/people/staff/sjt/Haskell_craft/Palin.hs}.
...
\begin{itemize}
\item \texttt{Minesweeper.hs}: a simple interface -- input a row and column character to uncover a square; run \texttt{playGrid}.
\item \texttt{Minesweeper2.hs}: implements \texttt{q}, \texttt{s}, \texttt{m}, \texttt{u}, \texttt{r}. To play, run \texttt{playGame m n} where \texttt{m} is the number of mines and \texttt{n} the size of the (square) board.
\item \texttt{Minesweeper3.hs}: adds \texttt{a} and \texttt{t} to the commands above. Played the same way as \texttt{Minesweeper2}.
\item \texttt{Minesweeper4.hs}: adds \texttt{h}. Played the same way as \texttt{Minesweeper2}.
\item \texttt{Minesweeper5.hs}: a further refinement of \texttt{Minesweeper4}, played the same way.
\item \texttt{MineRandom.hs}: generates a random starting grid, for use with the versions above.
\end{itemize}
