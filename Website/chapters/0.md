...
-   One area where Haskell has been particularly successful is in providing the substrate for developing embedded *domain-specific languages* (DSLs) and Chapter [Domain-Specific Languages](19.md#dsls) is devoted to this. It begins by explaining the reasons for developing DSLs, and then examines the difference between shallow and deep embeddings, looking at the examples of pictures and regular expressions. It concludes with a discussion of *monadic* DSLs, exemplified by naming in a pictures DSL and by the generators of QuickCheck.

-   The text has been *reordered* so that some material comes earlier than it did previously.[^1] Material on `data` types comes substantially earlier, with enumerated types coming into Section [Defining types for ourselves: enumerated types](4.md#EnumTypes) and non-recursive types into Section [Introducing algebraic types](5.md#introAlg). Programming for IO interactions is introduced in Chapter [Playing the game: I/O in Haskell](8.md#io) to support the Rock - Paper - Scissors example: this treatment simply presents the `do` notation as the way that IO is programmed, and delays an explanation of the underlying mechanism to Chapter [Programming with monads](18.md#actions). Finally, local definitions first come into Section [Solving a problem in steps: local definitions](4.md#localDefs).
...
-   The discussions in Chapter [Time and space behaviour](20.md#behaviour) on *performance* have been supplemented with material on how to measure the performance of real programs in GHC.
...
:   The book emphasises program proof, in a thread starting with Chapter [Reasoning about programs](9.md#reasoningProgs), and followed up in Sections [verifGen](11.md#verifGen), [proofAlgTypes](14.md#proofAlgTypes), [Commentary](16.md#adtCommentary) and [Proof revisited](17.md#lazyProof).

Program performance

:   Similarly, Chapter [Time and space behaviour](20.md#behaviour) gives a self-contained treatment of program time and space behaviour.
...
:   In this example, we see `Move` as a first example of an enumerated type in Section [Defining types for ourselves: enumerated types](4.md#EnumTypes), and strategies as an example of 'functions as data' in Section [Rock - Paper - Scissors: strategies](8.md#strategiesRPS). It also provides an example of an IO interaction later in that chapter.

Calculator

:   The calculator example begins in Section [Recursive algebraic types](14.md#recTypes) with the `Expr` type, a recursive algebraic data type describing numerical expressions. Chapter [Abstract data types](16.md#adt) introduces the abstract type of `Store` used to model the values of the variables. In Section [Case study: parsing expressions](17.md#parsing) we see how to parse text into numerical expressions and finally in Section [The calculator](18.md#interactiveCalc) we give an interactive read-evaluate-print loop for calculation.

Library database

:   When this is introduced in Section [A library database](5.md#library) it shows how to build programs over lists using list comprehensions, without using explicit recursion.

Supermarket billing

:   This example comes in Section [Extended exercise: supermarket billing](6.md#superBill) and again illustrates the utility of the list library functions independently of explicit recursive definitions.

Text processing

:   In text processing, Section [Example: text processing](7.md#textProc), we present an example where explicit recursion over lists is necessary, in contrast to the previous two.

Regular expressions

:   Regular expressions are first introduced as an example of 'functions as data', and then discussed again in Chapter [Domain-Specific Languages](19.md#dsls) when a deep embedding is developed to contrast with the earlier treatment, a shallow embedding.

Huffman codes

:   Chapter [Case study: Huffman codes](15.md#codes) is devoted to this multi-module application. The principal purpose of this is to show a larger example of programming in practice.

Other examples

:   Other examples of simulation, relations and graphs are used to illustrate lazy evaluation in Chapter [Lazy programming](17.md#lazy).
