...
- Functional languages are used to build secure, error-free, systems that are used in practice by thousands of people every day. For example, the [*Xmonad*](https://xmonad.org) window manager for linux systems is written entirely in Haskell, and the [*Cryptol*](https://github.com/GaloisInc/cryptol) language, used to design cryptographic systems in C and VHDL, is itself implemented using Haskell. At a larger scale, the investment bank Standard Chartered runs its Markets trading division on a Haskell dialect spanning millions of lines of code, the fintech company Mercury processes hundreds of billions of dollars a year in transactions on a Haskell backend, and [Plutus](https://docs.cardano.org/developer-resources/smart-contracts/plutus), the smart-contract language for the Cardano blockchain, is a dialect of Haskell, and indeed the code for the Cardano blockchain node is itself written in Haskell.
...
- In fact, most modern languages, including Java, C and C++, *are* already functional, in the sense that they contain 'first class' functions, or functions as data, defined using a lambda. Users of these languages stress the value of having access to higher-order functions when defining as general an API as possible for a library. When concurrent threads are to be used, an immutable data type can provide a safe and effective approach: as observed by Pat Helland, [*immutability changes everything*](https://queue.acm.org/doi/10.1145/2857274.2884038).

- Finally, it may well be that you will find yourself *working with a functional language* after all, even if it is not Haskell. Microsoft's F# functional language is now open source and cross-platform as part of .NET, and is in increasing use in a number of sectors, particularly finance. Erlang, the concurrent functional language, is used to provide scalable infrastructure for many web-based systems such as WhatsApp, and its virtual machine now also underlies Elixir, a newer functional language built on the same technology.
...
Haskell began life in the late 1980s as an intended standard language for lazy functional programming, and since then it has gone through various changes and modifications. This text is written in Haskell 2010, which remains the latest official language standard: in practice it has proved a remarkably stable base, with GHC instead evolving through language extensions -- curated since 2021 into *language editions* such as `GHC2021` and `GHC2024` -- so the parts of the language discussed here remain stable in current GHC.
...
The *Haskell Foundation*, established in 2020, now provides much of the ecosystem's shared infrastructure: it funds maintenance of core tools and libraries, and runs an annual community survey tracking how Haskell is actually used in practice ([State of Haskell 2025](https://discourse.haskell.org/t/state-of-haskell-2025-results/13755)). A Haskell Foundation working group has recently begun work on a revised Haskell 2010 Report, the first update to the standard in over a decade.
...
[Playing the game: I/O in Haskell](8.md#io) shows how simple terminal IO is handled in Haskell: to do this the `do` notation for writing programs of type `(IO a)` is introduced as an extension of Haskell's syntax, with the explanation of what underlies it being postponed to [Abstraction: functors, monads and folding](19.md#actions), where monads are covered. An interactive version of the Rock - Paper - Scissors game is used to illustrate IO in practice.
...
Haskell programs can perform input and output by means of the `IO` types, first introduced in [Playing the game: I/O in Haskell](8.md#io). [I/O programming](18.md#io-programming) revises this, and illustrates some larger-scale examples, including an interactive front-end to the calculator. The foundations of the `do` notation lie in monads, which can also be used to do action-based programming of a number of different flavours, examined in [Abstraction: functors, monads and folding](19.md#actions).

Domain--specific languages are one area where Haskell has been used very effectively. [Domain-Specific Languages](20.md#dsls) explains what is a DSL, how a DSL can be embedded in a language like Haskell, and the distinction between shallow and deep embeddings. Example DSLs build on earlier examples, as well as introducing new ones such as the generator language for QuickCheck, which neatly illustrates a monadic DSL.

The text continues with an examination in [Time and space behaviour](21.md#behaviour) of program behaviour, by which we mean the time taken for a program to compute its result, and the space used in that calculation. It also explains the basics of how to measure the run-time behaviour of Haskell programs. [Conclusion](22.md#conclusion) concludes by surveying various applications and extensions of Haskell as well as looking at further directions for study. These are backed up with web and other references.
...
The Haskell code for all the examples in the book, as well as other background materials, can be downloaded from the book's [GitHub repository](https://github.com/simonjohnthompson/haskellcraft).

## What is new in this online edition? {#what-is-new-in-this-online-edition .unnumbered}

This is a revision of the 2011 third edition, not a fourth edition in its own right: the core text, examples and case studies are unchanged in substance. What has changed:

- The book is now freely available online under a Creative Commons licence (CC BY-NC-SA 4.0), alongside the PDF, as a web edition built with mdBook, with the source for both available from the book's [GitHub repository](https://github.com/simonjohnthompson/haskellcraft).

- The recommended toolchain has moved on: *GHCup* is now the single recommended installer for GHC, Cabal and the Haskell Language Server (HLS), and VS Code with HLS is the recommended editor setup; [Getting started with Haskell programming](2.md#getStart) and its GHCi transcripts and screenshots have been brought up to date to match. Readers who want to try the book's code without installing anything can use the accompanying dev container (GitHub Codespaces or Gitpod).

- The code accompanying the book has been brought into line with current GHC and `base`: for example the `MonadFail` class, split out of `Monad` since `base-4.13`, and the `Foldable` class, are now covered directly, and every example compiles cleanly under a current GHC.

- Guidance on finding libraries and getting help, in [Finding your way around the Haskell libraries](6.md#HaskellLibs), has been refreshed: Hackage, Stackage snapshots, Haddock and Hoogle for libraries; the Haskell Discourse, Haskell Cafe and Stack Overflow for questions.

- Dead links to `haskellcraft.com` have been retired, with that material folded into the book and its GitHub repository.

- A number of case-study programs have been refreshed: the graphical *Minesweeper* example in [Problem solving and further examples](further.md#further) now uses the actively-maintained `gloss` library in place of a graphics package that no longer exists, and several figures -- including the class-hierarchy diagram in [Overloading, type classes and type checking](13.md#classes) -- have been replaced with freely-licensed originals.

As noted above, this revision was itself carried out with the assistance of an LLM (Claude Code, using Claude Sonnet 5) --- itself a small illustration of how much the wider software landscape has changed since 2011.
...
- QuickCheck is presented as complementary to *proof*, which has always been included in the book: QuickCheck can provide strong evidence for a property holding, while proof can establish its validity. Some custom generators are supplied in the code base for the book so that programmers can test their code before the details of how generators are defined are discussed in [Domain-Specific Languages](20.md#dsls).
...
- The discussions in [Time and space behaviour](21.md#behaviour) on *performance* have been supplemented with material on how to measure the performance of real programs in GHC.

- A collection of *project suggestions* has been added as an appendix.
...
: The book emphasises program proof, in a thread starting with [Reasoning about programs](9.md#reasoningProgs), and followed up in [Verification and general functions](11.md#verifGen), [Reasoning about algebraic types](14.md#proofAlgTypes), [Commentary](16.md#adtCommentary) and [Proof revisited](17.md#lazyProof).

Program performance

: Similarly, [Time and space behaviour](21.md#behaviour) gives a self-contained treatment of program time and space behaviour.

The case studies in the book are designed to illustrate particular points and constructs as well as to give examples of larger programs than single function definitions.<a id="CaseStudies"></a>

Pictures

: This is used to show the utility of lists in modelling, as well as the value of the higher-order and polymorphic functions over lists, such as `zipWith`, `map` and `(++)`. Pictures are also used to illustrate shallow and deep embeddings of domain-specific languages in [Domain-Specific Languages](20.md#dsls), with a variant of 'named' pictures giving an example of a monadic DSL.
...
: Regular expressions are first introduced as an example of 'functions as data', and then discussed again in [Domain-Specific Languages](20.md#dsls) when a deep embedding is developed to contrast with the earlier treatment, a shallow embedding.
...
Moving forward to 2026, I would reiterate all my thanks, and acknowledge colleagues and students at Eötvös Loránd University in Budapest who have given me abundant feedback on courses that I have taught there; in particular Viktor Bense went above and beyond in providing invaluable feedback on this text and my online lectures.
...
*Simon Thompson*\
*Canterbury, December 2010 and September 2026*
