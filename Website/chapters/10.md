...
![](Pictures/mapType.png){width="4in"}
...
![](Pictures/filterType.png){width="4in"}
...
10. Using `iter` and `double` define a function which on input `n` returns $\texttt{2}^{\texttt{n}}$; remember that $\texttt{2}^{\texttt{n}}$ means one multiplied by two `n` times.
...
![](Pictures/foldrType1.png){width="4in"}
