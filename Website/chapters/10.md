...
We have already seen one aspect of this generality in **polymorphism**, under which the same program can be used over many different types. The prelude functions over lists introduced in Chapter [Data types, tuples and lists](5.md#tupleList) provide many examples of this including `length`, `++` and `take`.
...
We also saw mapping in Chapter [Data types, tuples and lists](5.md#tupleList) in our first example of a list comprehension which was to double every element of a list of integers.
...
Selecting all the elements of a list with a given property is also common. Chapter [Data types, tuples and lists](5.md#tupleList) contains the example of the function which selects the digits from a string
...
The first example of primitive recursion in Chapter [Defining functions over lists](7.md#defFunLists) was `sum`,
...
A common pattern in the text processing example of Chapter [Defining functions over lists](7.md#defFunLists) is to take or drop items from a list while they have some property. A first example is `getWord`,
...
We give a full explanation of how this type is derived in Section [Polymorphic type checking](13.md#polyTypeCheck).
...
where `take n xs` and `drop n xs` are intended to take or drop `n` elements from the front of the list `xs`. These functions are defined in Chapter [Defining functions over lists](7.md#defFunLists).

Also in Chapter [Defining functions over lists](7.md#defFunLists) we looked at the example of text processing, in which lists were split to yield words and lines. The functions `getWord` and `dropWord` defined there were *not* polymorphic, as they were designed to split at whitespace characters.
...
4.  The function `getLine` of Chapter [Defining functions over lists](7.md#defFunLists) has a polymorphic type -- what is it? How could you generalize the test in this function? If you do this, does the type of the function become more general? Explain your answer.
...
We have discussed the `Picture` type and the functions over it already in Chapter [Introducing functional programming](1.md#intro), Sections [A second example: pictures](2.md#picModuleEx) and [Solving a problem in steps: local definitions](4.md#localDefs) and Chapter [Programming with lists](6.md#progLists). In this chapter we have seen that we can define `flipV` using `map`, and `beside` using `zipWith`. The exercises that follow pick up other examples.
...
3.  Revisit the exercises of Section [Extended exercise: supermarket billing](6.md#superBill), where the supermarket billing example was developed, and re-implement your solutions using the prelude and library functions `map`, `filter` and so on.
...
We introduced the Rock - Paper - Scissors game in Section [Defining types for ourselves: enumerated types](4.md#EnumTypes) and built on that by defining strategies in Section [Rock - Paper - Scissors: strategies](8.md#strategiesRPS) and showing how to play the game interactively in Section [Rock - Paper - Scissors: playing the game](8.md#playingRPS).
