...
where `...` will be the function to invert the colour in a single line. To invert every character in a line -- which is itself a list of characters -- we will again use `map`. The function mapped is `invertChar`, first defined in [picturesAgain](6.md#picturesAgain). This gives the definition
...
We have already discussed pictures in [Introducing functional programming](1.md#intro) and then in [A second example: pictures](2.md#picModuleEx), [picturesAgain](6.md#picturesAgain) and [hofPicture](#hofPicture), and these sections present various operations over the `Picture` type.
