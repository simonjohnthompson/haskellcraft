...
>
> different ways in Haskell. We talked in Section [Generic functions: polymorphism](6.md#poly) of a type `t_1` being an instance of a *type* `t_2`, when we can substitute for a type variable in `t_2` to give `t_1`. Here we have talked about a type being an instance of a *class*. It should be clear which one we mean from the context of a discussion, but it's helpful to bear this 'overloading' of terminology in mind.
...
The `elem` example in Section [Why overloading?](#whyOver) will have the type
...
and so on. Many of the functions we have defined already use equality in an overloaded way. We can use GHCi to deduce the most general type of a function, such as the `books` function from the library database of Section [A library database](5.md#library), by commenting out its type declaration in the script, thus
...
Clearly from this definition there is nothing specific about books or people and so it is polymorphic, if we can compare objects in the first halves of the pairs for equality. This condition gives rise to the context `Eq a`. Finally from Section [A library database](5.md#library), as we saw for `books`,
...
> **Type error messages**
>
> We first saw the example of a type error message mentioning 'instances' in Section [Errors and error messages](2.md#errors) where the application `4 double` gave rise to the error message
>
> ``` {.haskell}
...
We will explain the type `Ordering` and the function `compare` in Section [A tour of the built-in Haskell classes](#haskellClasses).

A simple example of a function defined over types in the class `Ord` is the insertion sort function `iSort` of Chapter [Defining functions over lists](7.md#defFunLists). Its most general type is
...
style, explored in Section [Algebraic types and type classes](14.md#oop).
...
1.  How would you make `Move`, playing cards (as defined in Section [Extended exercise: cards and card games](6.md#cardGames)), and triple types, `(a,b,c)`, into `Info` types?
...
Most Haskell types belong to these equality and ordering classes: among the exceptions are function types, and some of the abstract data types we meet below in Chapter [Abstract data types](16.md#adt).
...
In fact we discuss how some function types might be shown in Section [DSLs for computation: generating data in QuickCheck](19.md#qcGens).
...
> **`Functor` and `Monad`**
>
> We talk about the `Functor` and `Monad` classes in Chapter [Programming with monads](18.md#actions).
...
There is one case where we *do* need to use type declarations or annotations: this is in resolving ambiguity due to overloading (we talked briefly about this earlier, in Section [ambiguity](#ambiguity)).
...
As we saw in Section [Monomorphic type checking](#monoCheck), function application is central to expression formation. This means that type checking also hinges on function applications.
...
In Section [Folding and primitive recursion](10.md#foldPrimRec) we introduced the `foldr` function
...
7.  \[Harder\] Recalling the definitions of `curry` and `uncurry` from Section [Currying and uncurrying](11.md#curry), what are the types of
