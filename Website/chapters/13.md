...
<a id="ix-13-errors-type-error-messages"></a>
...
![The Haskell Base classes[^2]](Pictures/Base-classes.png){#ClassesDiagram width="3.5in"}

<a id="ClassesDiagram"></a>

<a id="ix-13-haskell-base-classes-diagram"></a>

<a id="ix-13-classes-diagram-for-haskell"></a>
...
which converts a value into a string is all that is needed. The class contains default definitions of `showsPrec` from `show` and vice versa. Further details about how to exploit the subtleties of `showsPrec` can be found in [Hudak et al. (2000)](bibliography.md#gentle98).
...
>
> In Haskell a class is a collection of types. Other languages such as `C++` make a type and a class the same thing. Under that approach, introducing the class of visible objects would effectively give us a type[^4] `ShowType`. This class would be characterized by having the function
>
> ``` haskell
...
[^2]: By Leftaroundabout - Own work, CC BY-SA 4.0, <https://commons.wikimedia.org/w/index.php?curid=63712544>

[^3]: Apart from (de)coding of Char, `take`, `drop` and so forth.

[^4]: In `C++` teminology this would be an abstract base class, with `Bool` etc. inheriting and being forced to implement the operations of that class.
