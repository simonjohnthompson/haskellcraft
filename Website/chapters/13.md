...
The `elem` example in Section \[REF:whyOver\] will have the type
...
and so on. Many of the functions we have defined already use equality in an overloaded way. We can use GHCi to deduce the most general type of a function, such as the `books` function from the library database of Section \[REF:library\], by commenting out its type declaration in the script, thus
...
Clearly from this definition there is nothing specific about books or people and so it is polymorphic, if we can compare objects in the first halves of the pairs for equality. This condition gives rise to the context `Eq a`. Finally from Section \[REF:library\], as we saw for `books`,
...
We will explain the type `Ordering` and the function `compare` in Section \[REF:haskellClasses\].

A simple example of a function defined over types in the class `Ord` is the insertion sort function `iSort` of Chapter \[REF:defFunLists\]. Its most general type is
...
### `infoCheck`: a QuickCheck clone {#infocheck-a-quickcheck-clone .unnumbered}
...
1.  How would you make `Move`, playing cards (as defined in Section \[REF:cardGames\]), and triple types, `(a,b,c)`, into `Info` types?
...
Most Haskell types belong to these equality and ordering classes: among the exceptions are function types, and some of the abstract data types we meet below in Chapter \[REF:adt\].
...
There is one case where we *do* need to use type declarations or annotations: this is in resolving ambiguity due to overloading (we talked briefly about this earlier, in Section \[REF:ambiguity\], page \[PAGEREF:ambiguity\]).
...
As we saw in Section \[REF:monoCheck\], function application is central to expression formation. This means that type checking also hinges on function applications.
...
In Section \[REF:foldPrimRec\] we introduced the `foldr` function
...
In type checking a function definition like `(fdef)` on page \[PAGEREF:fdef\] above we have to obey rules similar to the monomorphic case.
...
7.  \[Harder\] Recalling the definitions of `curry` and `uncurry` from Section \[REF:curry\], what are the types of
