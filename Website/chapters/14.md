...
1.  Reimplement the library database of [A library database](5.md#library) to use an algebraic type like `People` rather than a pair. Compare the two approaches to this example.
...
In [proofAlgTypes](#proofAlgTypes) we'll look at specifying QuickCheck properties for functions over algebraic types, including `assoc`, as well as showing how some of these can be proved by induction.
...
Guided by this, we can give a higher-order function which *joins together* two functions defined on `a` and `b` to a function on `Either a b`. The definition follows, and is illustrated in Figure [Joining together functions.](#either).
...
These two operations are illustrated in Figure [Error-handling functions.](#errPic), and we define them now.
