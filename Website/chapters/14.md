...
-   composite types: tuple types, `(,,…,)`; list types, `[]`; and function types, `( -> )`; where ``, ..., `` are themselves types,
...
will be a member of the type `Typename` if is in for `j` ranging from `1` to . Reading the constructors as functions, the definition `(Typename)` gives the constructors the following types
...
![Joining together functions.](Pictures/either1){width="3in"}
...
l4cm ![](Pictures/FishChips.png){width="4cm"}
...
<a id="nMovObj"></a>
We can also reflect an object in the x-axis (the horizontal axis) or the y-axis (the vertical), or rotate a figure through $180^{\circ}$ around the origin (the point where the axes meet). The default definition of `rotate180` works by reflecting first in the y-axis and then the x, as we did with the `Picture` type in Chapter [Introducing functional programming](1.md#intro).
