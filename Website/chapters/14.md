...
We revisit the `Maybe` type in [Monads: languages for functional programming](19.md#monadFP) where we see that it is an example of a more general programming structure, a monad. In particular there we examine the relationship between the function `mapMaybe` and the `map` function over lists.
