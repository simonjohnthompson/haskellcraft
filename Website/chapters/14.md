...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/NewTree.png" alt="An example of a tree of integers." style="width:3in"><span class="img-wrapper"><img src="Pictures/NewTree.png" alt="An example of a tree of integers."></span></label>
...
-   composite types: tuple types, `(t_1,t_2,…,t_n)`; list types, `[t_1]`; and function types, `(t_1 -> t_2)`; where `t_1`, ..., `t_n` are themselves types,

-   algebraic types, including enumerated types, as introduced in Section [Defining types for ourselves: enumerated types](4.md#EnumTypes), and product and sum type, first given in Section [Introducing algebraic types](5.md#introAlg).
...
-   The types can be **recursive**; we can use the type we are defining, `Typename`, as (part of) any of the component types, as in the definition of numeric trees "`NTree`s":
    ...
    ```haskell
    data NTree = NilT |NTree
                 Node Integer NTree NTree
    ```

    an illustration of a tree from `NTree` is given in Figure [An example of a tree of integers.](#treeFig). Recursion in `data` type definitions gives us lists, trees and many other useful data structures.

-   The name of the type being defined can be followed by one or more type variables which may be used on the right-hand side, making the definition **polymorphic**. An example built-into Haskell is the `Maybe` type,
...
the name of the type, an equals sign and then the **constructors** of the type being defined. The name of the type and the names of constructors begin with capital letters.
...
Each `Con_i` is a constructor, followed by k\_i types, where k\_i is a non-negative integer which may be zero. We build elements of the type `Typename` by applying these constructor functions to arguments of the types given in the definition, so that

```haskell
Con_i v_i1 ... v_ik_i
```

will be a member of the type `Typename` if v\_ij is in t\_ij for `j` ranging from `1` to k\_i. Reading the constructors as functions, the definition `(Typename)` gives the constructors the following types
...
```haskell
data Season = Spring | Summer | Autumn | WinterSeason
```
...
As we discussed in Section [A tour of the built-in Haskell classes](13.md#haskellClasses), Haskell has a number of built-in classes including `Eq`, `Ord`, `Enum`, `Show`, and `Read`. When we introduce a new algebraic type it's possible to `derive` instances of these classes for the new type, like this:
...
1.  Reimplement the library database of Section [A library database](5.md#library) to use an algebraic type like `People` rather than a pair. Compare the two approaches to this example.

2.  The library database of Section [A library database](5.md#library) is to be extended in the following ways.
...
```haskell
LitAddSub
data Expr = Lit Integer |Expr
            Add Expr Expr |
            Sub Expr Expr
```

Similarly, a tree is either nil or is given by combining a value and two sub-trees. For example, the number `12` and the trees in Figure [Two trees.](#treeFig2) are assembled to give the tree in Figure [An example of a tree of integers.](#treeFig).

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/NewTwoTrees.png" alt="Two trees." style="width:3in"><span class="img-wrapper"><img src="Pictures/NewTwoTrees.png" alt="Two trees."></span></label>
...
Each of these functions will be defined in the same way, using **primitive recursion**. As the type is itself recursive, it is not a surprise that the functions which handle the type are also recursive. Also, the form of the recursive definitions follows the recursion in the type definition. For instance, to evaluate an operator expression we work out the values of the arguments and combine the results using the operator.

```haskell
eval :: Expr -> Integereval
...
-   At the recursive cases, the values of the function at the

    sub-expressions from which the expression is formed -- `eval e1` and `eval e2` here -- can be used in calculating the result.
...
Trees of integers like that in Figure [An example of a tree of integers.](#treeFig) can be modelled by the type
...
The null tree is given by `NilT`, and the trees in Figure [Two trees.](#treeFig2) by
...
```haskell
sumTree,depth :: NTree -> IntegersumTreedepth
...
The operation of addition over the integers is associative, so that the way in which an expression is bracketed is irrelevant to its value. We can, therefore, decide to bracket expressions involving '`+`' in any way we choose. The aim here is to write a program to turn expressions into right bracketed form, as shown in Figure [Rearranging additions](#associate) and in the following table:
...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Associate.png" alt="Rearranging additions" style="width:3in"><span class="img-wrapper"><img src="Pictures/Associate.png" alt="Rearranging additions"></span></label>
...
The equation `(Add.2)` will only be applied to the cases where `(Add.1)` does not apply -- this is when `e1` is either a `Sub` or a `Lit` expression. This is always the case in pattern matching; the *first* applicable equation is used.

When we use primitive recursion we can be sure that the recursion will **terminate** to give an answer: the recursive calls are only made on smaller expressions and so, after a finite number of calls to the function, a base case will be reached.
...
In Section [proofAlgTypes](#proofAlgTypes) we'll look at specifying QuickCheck properties for functions over algebraic types, including `assoc`, as well as showing how some of these can be proved by induction.
...
13. It is possible to extend the type `Expr` so that it contains *conditional* expressions, `If b e1 e2`, where `e1` and `e2` are expressions, and `b` is a Boolean expression, a member of the type `BExp`,
    ...
    ```haskell
    eval  :: Expr -> Integer eval
    bEval :: BExp -> BoolbEval
    ```
...
where the syntax `[a]`, `[]` and '`:`' is used for `List a`, `NilList` and `:::`. Note that we have given a **fixity declaration** for `:::` to give it the same fixity and associativity as `:`, we can therefore write expressions like this:
...
The trees of Section [Recursive algebraic types](#recTypes) carry numbers at each node; there is nothing special about numbers, and we can equally well say that they have elements of an arbitrary type at the nodes:

```haskell
NilNode
data Tree a = Nil | Node a (Tree a) (Tree a)Tree
              deriving (Eq,Ord,Show,Read)
```
...
```haskell
depth :: Tree a -> Integerdepth
depth Nil            = 0
depth (Node n t1 t2) = 1 + max (depth t1) (depth t2)
```

as do many of the functions defined in the exercises at the end of Section [Recursive algebraic types](#recTypes). One of these is the function collapsing a tree into a list. This is done by visiting the elements of the tree 'inorder', that is visiting first the left sub-tree, then the node itself, then the right sub-tree, thus:

```haskell
collapse :: Tree a -> [a]collapse
collapse Nil = []
...
```haskell
mapTree :: (a -> b) -> Tree a -> Tree bmapTree
mapTree f Nil = Nil
mapTree f (Node x t1 t2)
  = Node (f x) (mapTree f t1) (mapTree f t2)
```

We shall return to trees in Section [Search trees](16.md#srchTrees), where particular 'search' trees form a case study.
...
data Either a b = Left a | Right b
                  deriving (Eq,Ord,Read,Show)Either
```
...
Guided by this, we can give a higher-order function which *joins together* two functions defined on `a` and `b` to a function on `Either a b`. The definition follows, and is illustrated in Figure [either](#either).

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/either1.png" alt="Joining together functions." style="width:3in"><span class="img-wrapper"><img src="Pictures/either1.png" alt="Joining together functions."></span></label>
...
If we have a function `f::a -> c` and we wish to apply it to an element of , there is a problem: what do we do if the element is in the right-hand side of the `Either` type? A simple answer is to raise an `error`
...
1.  Investigate which of the functions over trees discussed in the exercises of Section [Recursive algebraic types](#recTypes) can be made polymorphic.
...
```haskell
data GTree a = Leaf a | Gnode [GTree a]GTree
```
...
-   we can *transmit* the error through a function, the effect of `mapMaybe`;

-   we can *trap* an error, the role of `maybe`.

These two operations are illustrated in Figure [errPic](#errPic), and we define them now.
...
```haskell
mapMaybe :: (a -> b) -> Maybe a -> Maybe bmapMaybe
...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/maybe1.png" alt="Error-handling functions." style="width:5in"><span class="img-wrapper"><img src="Pictures/maybe1.png" alt="Error-handling functions."></span></label>

<a id="errPic"></a>
```haskell
maybe :: b -> (a -> b) -> Maybe a -> bmaybe
...
We revisit the `Maybe` type in Section [Monads: languages for functional programming](18.md#monadFP) where we see that it is an example of a more general programming structure, a monad. In particular there we examine the relationship between the function `mapMaybe` and the `map` function over lists.
...
5.  The `Maybe` type could be generalized to allow messages to be carried in the `Nothing` part, thus:
    ...
    ```haskell
    data Err a = OK a | Error StringErr
    ```
...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/FishChips.png" alt="" style="width:4cm"><span class="img-wrapper"><img src="Pictures/FishChips.png" alt=""></span></label>
...
We return to the simulation example in Chapter [Abstract data types](16.md#adt).
...
```haskell
transform :: String -> String -> [Edit]transform
...
<a id="oop"></a>
We have reached a point where it is possible to explore rather more substantial examples of type classes, first introduced in Chapter [Overloading, type classes and type checking](13.md#typeClasses).

### Movable objects {#movable-objects .unnumbered}

```haskell
data Vector = Vec Float FloatVector

class Movable a whereMovable
  move      :: Vector -> a -> a
  reflectX  :: a -> a
  reflectY  :: a -> a
  rotate180 :: a -> a
...
instance Movable Point wherePoint
...
<a id="movOb1"></a>
```haskell
data Figure = Line Point Point |Figure
              Circle Point Float 
              deriving Show
...
We now work through the definitions, which are illustrated in Figures [Movable objects (1).](#movOb1) and [Movable objects (2).](#movOb2). Some moves will be dictated by vectors, so we first define
...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/moveVec.png" alt="" style="width:3in"><span class="img-wrapper"><img src="Pictures/moveVec.png" alt=""></span></label>
...
class (Movable b, Named b) => NamedMovable b  -- (3)NamedMovable

instance Movable a => NamedMovable (Name a)
```

<a id="nMovObj"></a>
We can also reflect an object in the x-axis (the horizontal axis) or the y-axis (the vertical), or rotate a figure through `180°` around the origin (the point where the axes meet). The default definition of `rotate180` works by reflecting first in the y-axis and then the x, as we did with the `Picture` type in Chapter [Introducing functional programming](1.md#intro).
...
For this instance we override the default definition of `rotate180` by changing the sign of both coordinates. This is a more efficient way of achieving the same transformation than the default definition.
...
and in the instance declaration of `Movable` for `Figure` given in Figure [Movable objects (2).](#movOb2) we use the corresponding operations on `Point`; for example,
...
This completes the demonstration that `NamedMovable (Name a)` holds when we know that `Movable a`. It is worth realising that this demonstration is produced automatically by the Haskell system -- we only need to type what is seen in Figure [Named movable objects.](#nMovObj).

This section has begun to illustrate how classes can be used in the software development process. In particular we have shown how our movable objects can be named in a way which allows reuse of all the code to move the objects.
...
The advice of Chapter [Reasoning about programs](9.md#reasoningProgs) about finding proofs can easily be carried over to the situation here. Now we give a representative example of a proof. We aim to prove for all finite trees `tr` that
...
mapTree f Nil = Nil  -- (mapTree.1)mapTree
mapTree f (Node x t1 t2)
  = Node (f x) (mapTree f t1) (mapTree f t2)   -- (mapTree.2)

collapse Nil = []  -- (collapse.1)collapse
collapse (Node x t1 t2)
...
discussed in Chapter [Higher-order functions](11.md#gen2). Examining the right-hand side now, we have
...
We have seen that not all functions are defined by primitive recursion. The example we saw in Section [Recursive algebraic types](#recTypes) was of the function `assoc`, which is used to rearrange arithmetic expressions represented by the type `Expr`. Recall that

```haskell
assoc (Add (Add e1 e2) e3)assoc
...
for all finite expressions `ex`. The induction principle for the `Expr`
...
> ```
>
> To be checkable, we need to be able to generate random values of the types `Expr`, `NTree` and `Tree a` (for types `a` for which random values can be generated). We have done this in the module available online, and we will explain the code in more detail in Chapter [Domain-Specific Languages](19.md#dsls).

**Exercises**

1.  Prove that the function `weather` from Section [Introducing algebraic types](5.md#introAlg) has the same behaviour as
...
2.  Is it the case that the `area` of each `Shape` from Section [Introducing algebraic types](5.md#introAlg) is non-negative? If so, give a proof; if not, give an example which shows that it is not the case.
...
4.  Show for all finite `NTree`s `tr` that
    ...
    The next two exercises refer back to the exercises of Section [Polymorphic algebraic types](#polyAlg).
