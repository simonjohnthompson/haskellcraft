...
We can give a general merge sort function, which works by **merging**, in order, the results of sorting the front and rear halves of the list.
...
We can write a top-level test for a system like this, by taking an example string, coding it and then decoding it. This should take us back to the string we started with, and indeed we can see this in the particular example in [The Main module of the Huffman coding system.](#mainMod). It is possible to change this to a QuickCheck property, which checks that coding/decoding is the identity for a whole collection of strings; we leave this as an exercise for the reader.
