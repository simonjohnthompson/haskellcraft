...
Suppose we are to build a calculator for numerical expressions, like those given by the `Expr` type of Section \[REF:recTypes\], but with variables included. The calculator is to provide the facility to set the values of variables, as well as for variables to form parts of expressions.
...
Figure \[REF:storePic\] illustrates the situation, and suggests that as well as giving a natural representation of the type of stores, there are two other benefits of type abstraction.
...
where we can see that the pattern of the definitions is similar, except that we have to 'unwrap' arguments of the form `(Store sto)` on the left-hand side, and 'wrap up' results using `Store` on the right-hand side. We look at a general mechanism for 'wrapping up' functions in the example of the `Set` ADT in Section \[REF:caseSets\].
...
6.  Design an ADT for the library database, first examined in Chapter \[REF:tupleList\].
...
3.  Define QuickCheck properties which will test the queue implementations given here. We will show how to define the appropriate generators needed to perform the tests in Chapter \[REF:dsls\].
...
7.  \[Harder\] Examine how priority queues could be used to implement the Huffman coding system in Chapter \[REF:codes\].
...
2.  Design a signature for an abstract type of library databases, as first introduced in Chapter \[REF:tupleList\].

3.  Design a signature for an abstract type of indexes, as examined in Section \[REF:indexExam\].

Simulation
----------

We first introduced the simulation example in Section \[REF:algDesign\], where we designed the algebraic types `Inmess` and `Outmess`. Let us suppose, for ease of exposition, that the system time is measured in minutes.
...
3.  Define QuickCheck properties which will test the simulation system. We will show how to define the appropriate generators needed to perform the tests in Chapter \[REF:dsls\].
...
We discussed the signature of the abstract data type earlier, in Section \[REF:adtDesign\], but we repeat it here.
...
Note that the types of `insTree`, `delete`, `minTree` and `join` contain the context `Ord a`. Recall from Chapter \[REF:classes\] that this constraint means that these functions can only be used over types which carry an ordering operation, `<=`. It is easy to see from the definitions of these functions that they do indeed use the ordering, and given the definition of search trees it is unsurprising that we use an ordering in these operations. Now we look at the definitions in Figure \[REF:treeOps\] in turn.

Inserting an element which is already present has no effect,

while inserting an element smaller (larger) than the value at the root causes it to be inserted in the left (right) subtree. Figure \[REF:insertSearchTree\] shows `3` being inserted in the tree
...
To `join` two non-`Nil` trees `t1` and `t2`, where it is assumed that `t1` is smaller than `t2`, we pick the minimum element, `mini`, of `t2` to be the value at the root. The left sub-tree is `t1`, and the right is given by deleting `mini` from `t2`. Figure \[REF:deleteSearchTree\] shows the deletion of `7` from
...
2.  Define QuickCheck properties which will test the implementation of search trees. We will show how to define the appropriate generators needed to perform the tests in Chapter \[REF:dsls\].
...
7.  Using a modified version of search trees instead of lists, reimplement the indexing software of Section \[REF:indexExam\].
...
The principal definitions over `Set a` are given in Figures \[REF:setDefs1\] and \[REF:setDefs2\]. At the start of the file we see that we import the library `List`, but as there is a definition of `union` in there we have to hide this on import, thus,
...
We now run through the individual functions as they are implemented in Figures \[REF:setDefs1\] and \[REF:setDefs2\]. In our descriptions we use curly brackets '`{`', '`}`', to represent sets in examples -- this is emphatically not part of Haskell notation.
...
We look at two examples of **family relations**, based on a relation `isParent` which we assume is given to us. We first set ourselves the task of defining the function `addChildren` which adds to a set of people all their children; we then aim to define the `isAncestor` relation. The full code for the functions discussed here is given in Figure \[REF:relFuns\].
...
The second task we set ourselves was to find the `isAncestor` relation. The general problem is to find the transitive closure of a relation, the function `tClosure` of Figure \[REF:relFuns\]. We do this by closing up the relation, so we add grandparenthood, great-grandparenthood and so forth to the relation until nothing further is added. We explain transitive closure formally later in this section.
