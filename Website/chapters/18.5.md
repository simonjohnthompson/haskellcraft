...
**19.4** Repeat some of the earlier examples and exercises using the `>>=` operator instead of a `do` expression. []{#ix-18.5-i-o}
...
- The value `fail` `s` corresponds to a computation which **fails**, giving the error message `s`. []{#ix-18.5-monad-failure-in-fail}

The laws are much clearer when stated in terms of a derived operator, `>@>`. []{#ix-18.5-se36849}
...
This operator generalizes function composition[^1] []{#ix-18.5-function-composition} in that it composes objects
...
Now we can state formally the rules that the operations of a monad should satisfy. []{#ix-18.5-monad-properties} First, `return` is an **identity**[]{#ix-18.5-identity} for the operator `>@>`:
...
and the operator `>@>` should be **associative**:[]{#ix-18.5-associativity}
...
::: {#ix-18.5-parsing-monad}
:::

::: {#ix-18.5-monad-parsing}
:::
...
where '`num :=`' corresponds to the '`<-`' and `do` puts a sequence of commands one after the other, as does the semi-colon. Remember, though, that this is a *single-assignment* language, so that `num`, `s1` and so on are names not variables! []{#ix-18.5-monad-and-imperative-programming} []{#ix-18.5-imperative-programming-and-monads}
...
::: {#ix-18.5-design-monads-and}
:::

::: {#ix-18.5-monad-advantages-of-monads}
:::
...
[^1]: In category theory, this operation is called **Kleisli composition**[]{#ix-18.5-kleisli-composition}.
