...
We first looked at I/O programming in Haskell using the `IO` types in Chapter \[REF:io\]. Something of type `IO t` is a *program* which will perform I/O and return a value of type `t`. Among the I/O primitives of Haskell are
...
In writing the program there are two cases: we first read an integer value, using the `getInt` function defined in Chapter \[REF:io\]. If we read zero then the result must be the sum so far, `s`; if not, we get the result by calling `sumInts` again, with the parameter set to `(s+n)`.

The program is written in the style we saw in Chapter \[REF:io\]: the function is *tail-recursive* -- being called as the last statement in the program -- and the *loop data* is used to carry the "sum so far": here `s` and `s+n`.
...
We introduced the `randomStrategy` in Chapter \[REF:io\], but pointed out there that it should properly be programmed using a monad, since it is a function which returns different values on different calls. We generate a random integer less than `n` based on the current time, which is accessible within the `IO` monad:
...
If we want to avoid `unsafePerformIO` we need to look again at the design of our case study. In Chapter \[REF:io\] defined strategies to belong to the type
...
-   In Section \[REF:recTypes\] we saw the introduction of the algebraic type of expressions, `Expr`, which we subsequently revised in Section \[REF:parsing\], giving
...
-   In Chapter \[REF:adt\] we introduced the abstract type `Store`, which we use to model the values of the variables currently held. The signature of the abstract data type is
...
-   In Section \[REF:parsing\] we looked at how to parse expressions and commands,
...
Well, we *can* do this, so long as the types have the analogue of the bind and return functions. In other words, we need to implement functions in a particular kind of *interface*, and we know from Chapter \[REF:classes\] that this is given in Haskell by the `Monad` class, like this:
...
Let's look at the example of programs that might fail, second in the list of examples above. We looked at these in Section \[REF:progErrs\] where we saw that the `Maybe` type was a good model for values including a potential failure:
...
where we are using the braces '{', '}' and explicit separator ':' to put the statements in the `do` program on a single line, as first introduced in Chapter \[REF:writingProgs\]. In a similar way, lists form a monad:
...
How does the monadic approach to parsing look in practice? Let's go back to the calculator example, and in particular the definition of `opExpParse` in Section \[REF:parsing\], page \[PAGEREF:opExpParse\]. If we redefine it monadically, we get
...
The structure here is exactly the same as that of `(sumTree)` on page \[PAGEREF:sumTree\]; we perform the operations on the components `x`, `t1` and `t2` (for the subtrees we use recursion) and then combine them in the result `(Node num nt1 nt2)`.
...
3.  Show how you can use a `State`-style monad in a computation to replace each element in a tree by a random integer, generated using the techniques of Section \[REF:infLists\].
