...
We first looked at I/O programming in Haskell using the `IO` types in Chapter [Playing the game: I/O in Haskell](8.md#io). Something of type `IO t` is a *program* which will perform I/O and return a value of type `t`. Among the I/O primitives of Haskell are
...
In writing the program there are two cases: we first read an integer value, using the `getInt` function defined in Chapter [Playing the game: I/O in Haskell](8.md#io). If we read zero then the result must be the sum so far, `s`; if not, we get the result by calling `sumInts` again, with the parameter set to `(s+n)`.

The program is written in the style we saw in Chapter [Playing the game: I/O in Haskell](8.md#io): the function is *tail-recursive* -- being called as the last statement in the program -- and the *loop data* is used to carry the "sum so far": here `s` and `s+n`.
...
We introduced the `randomStrategy` in Chapter [Playing the game: I/O in Haskell](8.md#io), but pointed out there that it should properly be programmed using a monad, since it is a function which returns different values on different calls. We generate a random integer less than `n` based on the current time, which is accessible within the `IO` monad:
...
If we want to avoid `unsafePerformIO` we need to look again at the design of our case study. In Chapter [Playing the game: I/O in Haskell](8.md#io) defined strategies to belong to the type
...
-   In Section [Recursive algebraic types](14.md#recTypes) we saw the introduction of the algebraic type of expressions, `Expr`, which we subsequently revised in Section [Case study: parsing expressions](17.md#parsing), giving

    ```haskell
    data Expr = Lit Integer | Var Var | Op Ops Expr ExprExpr
...
-   In Chapter [Abstract data types](16.md#adt) we introduced the abstract type `Store`, which we use to model the values of the variables currently held. The signature of the abstract data type is
...
-   In Section [Case study: parsing expressions](17.md#parsing) we looked at how to parse expressions and commands,
    ...
    ```haskell
    commLine :: String -> CommandcommLine
    ```
...
```haskell
command :: Command -> Store -> (Integer,Store)command
...
<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad1.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad1.png" alt=""></span></label>

with a **function** taking the result of this (of type `a`) into an `IO b`, that is an object of type `a -> IO b`,

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad2.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad2.png" alt=""></span></label>

We can join them together, passing the result of the first as an argument to the second, thus:

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad3.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad3.png" alt=""></span></label>

The result of putting them together is something which does some I/O before returning a value of type `b`:

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad4.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad4.png" alt=""></span></label>
...
Let's look at the example of programs that might fail, second in the list of examples above. We looked at these in Section [Modelling program errors](14.md#progErrs) where we saw that the `Maybe` type was a good model for values including a potential failure:
...
where we are using the braces '{', '}' and explicit separator ':' to put the statements in the `do` program on a single line, as first introduced in Chapter [Designing and writing programs](4.md#writingProgs). In a similar way, lists form a monad:
...
-   The value `fail` `s` corresponds to a computation which **fails**, giving the error message `s`.
...
This operator generalizes function composition[^1] in that it composes objects

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad5.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad5.png" alt=""></span></label>

to give

<label class="checkbox-label"><input class="checkbox-img" type="checkbox"><img src="Pictures/Monad6.png" alt="" style="height:1.2cm"><span class="img-wrapper"><img src="Pictures/Monad6.png" alt=""></span></label>

Note also that `return` is of this shape, as its type is `a -> m a`.

Now we can state formally the rules that the operations of a monad should satisfy. First, `return` is an **identity** for the operator `>@>`:
...
How does the monadic approach to parsing look in practice? Let's go back to the calculator example, and in particular the definition of `opExpParse` in Section [Case study: parsing expressions](17.md#parsing). If we redefine it monadically, we get
...
```haskell
sumTree :: Tree Integer -> St IntegersumTree
```
...
where '`num :=`' corresponds to the '`<-`' and `do` puts a sequence of commands one after the other, as does the semi-colon. Remember, though, that this is a *single-assignment* language, so that `num`, `s1` and so on are names not variables!
...
What else do we have to define to give the result? We need to identify the monad `State a` and to define the function which replaces an individual
...
3.  Show how you can use a `State`-style monad in a computation to replace each element in a tree by a random integer, generated using the techniques of Section [infLists](17.md#infLists).
