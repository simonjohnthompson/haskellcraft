...
An element of this type is either a leaf, or it's a node with a left subtree, a value of type `a`, and a right subtree. What do we do to apply `f` to everything of type `a` in the tree?
...
If we apply `fmap f` to a `Leaf`, we get a `Leaf`; if we apply `fmap f` to a `Node`, we recursively apply `fmap f` to the left subtree `t1` and the right subtree `t2`, and we apply `f` to the value `x` of type `a` sitting at the node, and that gives us a value of type `b`. In summary, a tree is a complicated structure, but at each internal node there is a value of type `a`, and what we have done is to apply `f` to transform that to something of type `b`.
...
instance Applicative Tree where
  ...
  liftA2 f Leaf _ = Leaf
  liftA2 f _ Leaf = Leaf
  liftA2 f (Node t1 x t2) (Node s1 y s2) =
    Node (liftA2 f t1 s1)
         f x y
         (liftA2 f t2 s2)
```

Let's look at the `Leaf` cases first. If you've got a `Leaf` on the left, there are no values on the left-hand side; if you've got a `Leaf` on the right, there are no values on the right-hand side. So in both those cases, we return a `Leaf`.

What about the remaining case, where we have two `Node`s --- we have a value `x` of type `a` and a value `y` of type `b`? We can apply `f` to those, giving us `f x y` of type `c`. Then, on the left, we recursively apply `liftA2 f` to `t1` and `s1`, the left subtrees; and on the right we apply it to `t2` and `s2`, the right subtrees. So only in the case where there are corresponding values of type `a` and `b` are we able to apply `f` --- otherwise we simply return a `Leaf`. (This definition is by analogy with `zipWith`; is there also a definition analogous to `zipAll`?)
