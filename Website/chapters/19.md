...
An element of this type is either a leaf, or it's a node with a left subtree, a value of type `a`, and a right subtree. What do we do to apply `f` to everything of type `a` in the tree? If we apply `fmap f` to a `Leaf`, we get a `Leaf`; if we apply `fmap f` to a `Node`, we recursively apply `fmap f` to the left subtree `t1` and the right subtree `t2`, and we apply `f` to the value `x` of type `a` sitting at the node, and that gives us a value of type `b`. In summary, a tree is a complicated structure, but at each internal node there is a value of type `a`, and what we have done is to apply `f` to transform that to something of type `b`.
...
Suppose we're lifting a function `f`. We take two arguments of type `Maybe a`. If we have `Just x` and `Just y`, we can apply `f` to `x` and `y` and return `Just (f x y)`. In every other case, we don't have something of both types `a` and `b`, so we return `Nothing`.
...
We can also define an instance for trees. Let's look at the `Leaf` cases first. If you've got a `Leaf` on the left, there are no values on the left-hand side; if you've got a `Leaf` on the right, there are no values on the right-hand side. So in both those cases, we return a `Leaf`.
...
And that's what we're going to talk about in the next segment: what monads are, and how they're defined in Haskell.
