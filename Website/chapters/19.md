...
The chapter then looks again at I/O in Haskell, particularly focussing on the `do` notation which we can use to write all sorts of different programs, including ones that manipulate state, that non-deterministically return more than one result, as well as programs that may fail, or raise exceptions.
...
The laws are much clearer when stated in terms of a derived operator, `>@>`. <a id="ix-19-se36849"></a>
...
**19.5** Show that sets and binary trees can be given a monad structure, as can the type:

``` haskell
data Error a = OK a | Error String
```

**19.6** For the monads `Id` , `[]` and `Maybe` prove the rules `(M1)` to `(M3)`. Also show that these rules hold for your implementations in the previous exercise.

**19.7** Prove the property `(M4)` using the laws `(M1)` to `(M3)`.

**19.8** Prove the following properties using the monad laws:
...
**19.9** Can you define a different monad structure over lists from that given above? Check that your definition has properties `(M1)` to `(M3)`.

**19.10** Write down the definitions of `map` and `join` over lists using list comprehensions. Compare them with the definitions of `fmap` and `join` given in the `do` notation in this section.

**19.11** Re-implement the parser for the calculator using the `do` construct (based on `>>=`) rather than `(>*>)` and `build`. Contrast the two approaches.

**19.12** Show that the type functions `(a,a)`, `([a],[a])` and `Tree a` are instances of `Functor`.
...
**19.13** Give a non-monadic definition of `numTree`, threading the state through the computation explicitly; compare your solution with the monadic version given here.

**19.14** Show how to look up the position of an element in a list
...
**19.15** Show how you can use a `State`-style monad in a computation to replace each element in a tree by a random integer, generated using the techniques of [Infinite lists](17.md#infLists).

**19.16** We can use monads to extend the case study of the calculator in a variety of ways. Consider how you would
