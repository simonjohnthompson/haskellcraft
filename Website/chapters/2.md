...
but we can also input these definitions directly into GHCi by prefacing them with the keyword `let`, as seen in [GHCiSessionMac](#GHCiSessionMac). Of course, this definition will be lost when we close GHCi, so if we want to keep a record of it, then we should add it to a Haskell module as well.
