...
In this text we describe the terminal-style interface to GHCi, illustrated in [A GHCi session](#GHCiSession), because this is common to Windows, Mac OS X and Linux.
...
To start GCHi on OS X and Linux, type `ghci` to the prompt in a terminal; to launch GHCi using a particular file, type `ghci` followed by the name of the file in question, as in
...
As we said in [Expressions and evaluation](1.md#exprEval), GHCi will evaluate expressions typed at the prompt. We see in [GHCiSessionMac](#GHCiSessionMac) the evaluation of `double (square 29)` to `1682`, like this
