...
How can we measure the complexity of the functions we write? One answer is to use an implementation of Haskell, which can be expected to produce some diagnostic information about evaluation. We examine this option in Section \[REF:foldingRev\], page \[PAGEREF:perfInPractice\], but for the moment we opt for a cleaner *model* of what is going on, and we choose to analyse the *calculations* we have been using. There are three principal measures we can use.
...
-   On the other hand, in the *worst* case, the first `ins` will take one step, the second two, and so on. By our counting argument in Section \[REF:funComplex\] the calculation will take `O(n^2)` steps.
...
-   In an *average* case, the `ins`'s will take a total of
    ...
    steps, whose sum is again `O(n^2)`, by our observation in Section \[REF:counting\] about the size of the sum `1+2...n`.
...
We first saw the `Set` abstract data type in Section \[REF:caseSets\], where we gave an implementation based on ordered lists without repetitions. Alternatively we can write an implementation based on arbitrary lists whose elements may
...
\[REF:srchTrees\]. We now compare the time complexity of these implementations, and summarize the results in the table which follows.
...
Recall the explanation of lazy evaluation in Section \[REF:lazyEval\], where we explained that parts of results are printed as soon as possible. Once part of a result is printed, it need no longer occupy any space. In estimating space complexity, we must be aware of this.
...
As we saw in Section \[REF:complexCalc\], the naive factorial function has ` O(n^1)` space complexity, as it forms the expression
...
How can we overcome this? We ought to make the intermediate values *needed*, so that they are calculated earlier. We do this here by adding a test; another method is given in Section \[REF:foldingRev\].
...
2.  Give an informal estimate of the complexity of the text processing functions in Chapter \[REF:defFunLists\].

Folding revisited
-----------------

One of the patterns of computation which we identified in Chapter \[REF:patternsGen\] is **folding** an operator or function into a list. This section examines the complexity of the two standard folding functions, and discusses how we can choose between them in program design. Before this we make a definition which expresses the fact of a function needing to evaluate an argument. This distinction will be crucial to our full understanding of folding.
...
As in Section \[REF:spaceBeh\], we see that the space complexity of this will be `O(n^0)`, since the elements of the list will be output as they are calculated. What happens when we fold a strict operator into a list? The definition of `fac` in Section \[REF:complexCalc\] can be rewritten as
...
As in Section \[REF:complexCalc\], the difficulty is that `foldl` as we have defined it is not strict in its second argument. Using the standard function `seq`
...
### Assessing performance in practice {#assessing-performance-in-practice .unnumbered}

We can get detailed performance information by *compiling* programs using `ghc` and passing the right parameters to the executable program. For the simple examples here, we define the expressions we are interested in Figure \[REF:sumDefs\]. This file is compiled using
...
The performance of `perfIA.out` is twice as bad (!), but for `perfIS.out`, as shown in Figure \[REF:perfISreport\], we see something much better. How do the two reports compare?
...
A calculation is given in Figure \[REF:fibCalc\], where different variables ``, `` and so on have been used for the different occurrences of the local variables `x` and `y`; this is not necessary but does make the different occurrences clearer.
...
is the longest common subsequence in the lists `take u xs` and `take v ys`. The function is given in Figure \[REF:maxSubFig\], and the definition is a straightforward adaptation of `mLen`.
...
-   the line-splitting algorithm we gave in Chapter \[REF:defFunLists\] is optimal in minimizing the sum of the inter-word spaces when the lines are justified;

-   the Huffman codes described in Chapter \[REF:codes\] are optimal in the sense of giving the shortest possible codings of files. We did not search all possible sets of codes in giving the Huffman code, rather we built it up from locally sensible choices.
...
3.  Finding the 'edit distance' between two strings was first discussed in Section \[REF:algDesign\] where we gave a dynamic programming solution to the problem. Show how you can give an efficient implementation of this algorithm using the techniques of this section, and also how you give a greedy solution to the problem. How do the two solutions compare?
