...
In what follows we will use the term the **natural numbers** for the non-negative integers: `0`, `1`, `2`, ....
...
> <a id="redefName"></a>
>
> The `max` function is defined in the prelude, `Prelude.hs`, and if a definition
>
> ``` {.haskell}
> max :: Integer -> Integer -> Integer
> ```
...
People and computers communicate using keyboard input and screen output, which are based on sequences of **characters**, that is letters, digits and 'special' characters like space, tab, newline and end-of-file. Haskell contains a built-in type of characters, called `Char`. Sequences or **strings** of characters form the Haskell `String` type.
...
In Haskell the numbers `4` and `2` belong to both `Int` and `Float`; they are overloaded, as discussed in [Overloading](#overlIntro). This is also true of some of the numeric functions; addition, for instance, has both the types
...
for a **conditional equation** built up from a number of clauses. In this layout, each **clause** starts on a new line, and the guards and results are lined up. Note also that by convention in this text we always specify the type of the function being defined. If any of the expressions `ei` or guards `gi` is particularly long, then the guard can appear on a line (or lines) of its own, like this
...
By convention, when we give names built up from more than one word, we capitalize the first letters of the second and subsequent words, as in '`maxThree`'. The same identifier can be used to name both a function and a variable, or both a type and a type constructor; we recommend strongly that this is *not* done, as it can only lead to confusion.
...
>
> name of the function in front of its argument(s) thus: `f v1 v2 … vn`. This binds more tightly than any other operator, so that `f n+1` is interpreted as `f n` plus `1`, `(f n)+1`, rather than `f` applied to `n+1`, `f (n+1)`. If in doubt, it is sensible to parenthesize each argument to a function application.
>
