...
<a id="float"></a>
In [ints](#ints) we introduced the Haskell type `Int` of integers. In calculating we also want to use numbers with fractional parts, which are represented in Haskell by the **floating-point** numbers which make up the type `Float`. We do not use `Float` heavily in what follows, and so this section can be omitted on first reading and used as reference material to be consulted when necessary.
...
In Haskell the numbers `4` and `2` belong to both `Int` and `Float`; they are overloaded, as discussed in [Overloading](#overlIntro). This is
