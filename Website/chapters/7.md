...
-   A **constructor** applied to a number of patterns `(C p1 p2,...,pn)`. To match this the argument must be an application of the constructor `C` to n arguments: each of the each `vk` must match the corresponding pattern `pk`. We'll look at this case again in the next section.
