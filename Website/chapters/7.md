...
Suppose we are asked to find the first digit in the string `st`, returning `’\0’` in case no digit is found. We can use the function `digits` of Section [List comprehensions](5.md#listComps) to give us the list of all the digits in the string: `digits st`. If this is not empty, that is if it matches `(x:_)`, we want to return its first element, `x`; if it is empty, we return `’\0’`.
