...
We can write what we have proved as a formula, $$\mbox{1 litre costs 27 pence \imp\ 4 litres cost \pounds 1.08}$$ where the arrow, , which is the logical symbol for **implication**, says that the second proposition **follows from** the first.

As we have seen, we prove an implication like $A\imp{}B$ by assuming $A$ in proving $B$. If we then find a proof of $A$, then knowing the implication will guarantee that $B$ is also valid.

Yet another way of looking at this is to see a proof of $A\imp{}B$ as a *process* for turning a proof of $A$ into a proof of $B$. We use this idea in proof by induction, as one of the tasks in building an induction proof is the induction step, where we prove that one property holds assuming another.
